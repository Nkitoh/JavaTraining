...
class LinkedList<E>{
	...
	// リストの要素を検索する
	//nullをリターンする場合は、呼び出した側でnullかどうかを検査する必要があるが、
	//例外をスローすれば、メソッドを作る側で、エラーを通知することができる。
		public LinkedList<E> find(E findValue) 
			throws  NoValueListException{
			LinkedList<E> ref;
			for (ref = this; ref != null; ref = ref.next) {
				if (ref.value.equals(findValue) )
					break;
			}
			if (ref == null)
				throw new  NoValueListException(findValue);
			return ref;
		}
}